...
export function calculateLLMCost(usage: TokenUsage): number {
  const inputCost = (usage.inputTokens / 1_000_000) * GPT5_MINI_PRICING.INPUT_PER_1M
  const outputCost = (usage.outputTokens / 1_000_000) * GPT5_MINI_PRICING.OUTPUT_PER_1M
  const cachedInputCost = ((usage.cachedInputTokens || 0) / 1_000_000) * GPT5_MINI_PRICING.CACHED_INPUT_PER_1M
 ...
 */
export function calculateTotalCost(llmUsage: TokenUsage, toolUsage: ToolUsage): number {
...
export function estimateMessageCost(
  ...
  const cachedSystemPromptCost = hasCachedSystemPrompt 
