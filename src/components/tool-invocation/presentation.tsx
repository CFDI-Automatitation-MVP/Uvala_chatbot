...
export function Presentation(props: PresentationProps) {
  ...
  const parsedSlides = React.useMemo(() => {
    ...
    const splitSlides = markdown.split(/^---\s*$/m).filter(slide => slide.trim());
    console.log('Split slides:', splitSlides);
    return splitSlides;
  }, [slides, markdown]);

  // Convert markdown to HTML (enhanced for presentations with design elements)
  const markdownToHtml = (markdown: string) => {
    return markdown
      .replace(/^# (.+)$/gm, '<h1 style="font-size: 3rem; font-weight: 800; text-align: center; margin-bottom: 3rem; color: inherit; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text; text-shadow: 0 4px 6px rgba(0,0,0,0.1);">$1</h1>')
      .replace(/^## (.+)$/gm, '<h2 style="font-size: 2.25rem; font-weight: 700; margin-bottom: 2rem; color: inherit; padding-left: 1rem; border-left: 4px solid #667eea; background: linear-gradient(90deg, rgba(102, 126, 234, 0.1) 0%, transparent 100%); padding: 1rem;">$1</h2>')
      .replace(/^### (.+)$/gm, '<h3 style="font-size: 1.75rem; font-weight: 600; margin-bottom: 1.5rem; color: inherit; position: relative; padding-bottom: 0.5rem;"><span style="position: absolute; bottom: 0; left: 0; width: 50px; height: 3px; background: linear-gradient(90deg, #667eea, #764ba2); border-radius: 2px;"></span>$1</h3>')
      .replace(/\*\*(.+?)\*\*/g, '<strong style="font-weight: bold; background: linear-gradient(135deg, #667eea, #764ba2); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">$1</strong>')
      .replace(/\*(.+?)\*/g, '<em style="font-style: italic; color: #667eea;">$1</em>')
      .replace(/`(.+?)`/g, '<code style="background: linear-gradient(135deg, #667eea20, #764ba220); padding: 0.3rem 0.6rem; border-radius: 0.5rem; font-family: \'JetBrains Mono\', \'Fira Code\', monospace; border: 1px solid #667eea40; font-size: 0.9em;">$1</code>')
      .replace(/^- (.+)$/gm, '<li style="margin-bottom: 0.75rem; padding-left: 0.5rem; position: relative; list-style: none;"><span style="position: absolute; left: -1rem; top: 0.5rem; width: 8px; height: 8px; background: linear-gradient(135deg, #667eea, #764ba2); border-radius: 50%; box-shadow: 0 2px 4px rgba(102, 126, 234, 0.3);"></span>$1</li>')
      .replace(/^(\d+)\. (.+)$/gm, '<li style="margin-bottom: 0.75rem; padding-left: 1rem; position: relative; list-style: none; counter-increment: custom-counter;"><span style="position: absolute; left: -1.5rem; top: 0; width: 24px; height: 24px; background: linear-gradient(135deg, #667eea, #764ba2); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; font-size: 0.8rem; font-weight: bold;">$1</span>$2</li>')
      .replace(/\n\n/g, '</p><p style="margin-bottom: 1.5rem; line-height: 1.8; color: inherit; font-size: 1.1rem;">')
      .replace(/^(?!<[h|l|c])/gm, '<p style="margin-bottom: 1.5rem; line-height: 1.8; color: inherit; font-size: 1.1rem;">');
  };
      ...
      <CardHeader className="pb-3 bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950 dark:to-purple-950 border-b border-blue-200 dark:border-blue-800">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="bg-white/80 dark:bg-black/80 backdrop-blur-sm px-3 py-2 rounded-full border border-blue-200 dark:border-blue-800 text-sm font-medium">
              ...
              <span className="text-gray-600 dark:text-gray-400">{parsedSlides.length}</span>
            </div>
        ...
        <div className="mt-4">
          <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 mb-2">
            <span>Progress</span>
            <span>{Math.round(((currentSlide + 1) / parsedSlides.length) * 100)}%</span>
          </div>
        ...
        {showDetails && (
          <CardDescription className="text-center text-sm mt-3 p-3 bg-white/50 dark:bg-black/50 rounded-lg border border-blue-200 dark:border-blue-800">
            ✨ Interactive presentation with {parsedSlides.length} slides. Click fullscreen for presentation mode.
          </CardDescription>
        )}
      </CardHeader>
