...
export function SimpleChatLearn() {
  ...
  const [_learnState, appStoreMutate, currentThreadId] = appStore(
    useShallow((state) => [state.learn, state.mutate, state.currentThreadId]),
  );

  // Force the dedicated learn model
  useEffect(() => {
  ...
  const {
    ...
    transport: new DefaultChatTransport({
      ...
      prepareSendMessagesRequest: ({ messages }) => ({
        body: {
          chatModel: appStore.getState().learn.chatModel,
          messages,
        },
      }),
    }),
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener("learnReset", handleReset);
  }, [resetChat]);
