  ...
  return (
    <div className="relative min-h-full">
      ...
      <div className="absolute inset-0 w-full h-full bg-black -z-10">
        ...
        <div className="absolute inset-0 bg-black/40 z-10" /> {/* Overlay for better contrast */}
      </div>
          ...
          return (
            <Card
              ...
              className={`relative bg-black/20 backdrop-blur-sm border-white/50 shadow-2xl text-white ${
                isActivePlan ? 'border-green-500 ring-2 ring-green-200' :
                plan.popular ? 'border-white ring-2 ring-white/30' : ''
              }`}
            >
              {isActivePlan && (
              ...
              <CardHeader>
                ...
                <CardDescription className="text-gray-300">{t(`plans.${plan.id}.description`)}</CardDescription>
              </CardHeader>

              <CardContent>
                <ul className="space-y-3 mb-6">
                  {t.raw(`plans.${plan.id}.features`).map((feature: string, index: number) => (
                ...
                {isActivePlan ? (
                  ...
                  >
                    {subscriptionLoading ? (
                      ...
                      t('subscribeTo', { plan: t(`plans.${plan.id}.name`) })
                    )}
                  </Button>
                )}
              </CardContent>
